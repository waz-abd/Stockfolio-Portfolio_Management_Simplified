...
console.log(newUser);     
  submitForm(JSON.stringify(newUser));
...
console.log(result);
if(result.match("Duplicate")) alert("Username Exists");
else if(result.match("successfully")){alert(result); window.location.href = 'index.html' ;}
else alert(result); 
}



//function to read from mariadb using node.js

async function fetchUser() {
            
 let username = document.getElementById('loginUsername').value;
 fetch(`http://stockfolio.me:3000/api/data/${username}`)
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        console.log("User data:", data); // Handle the retrieved data
